...
This file is the `webpack` entry point, so it's the place to reference any other webkacpable assets, including any
CoffeeScript, CSS, etc. that you need.

First, by using `require` to get `index.html`, we are effectively telling `webpack` to copy index.html into the output
directory.
...
component's view. Instead of having one global stylesheet, each component has its own.

For example, `/src/home/view.coffee` could include a line `require './home.scss'`.

`starter-kit` comes with the [Skeleton](http://getskeleton.com/) CSS framework. Skeleton was chosen because it's
lightweight (11 kb), but it includes a fluid grid system, attractive typography, and simple, flat `input` styling.

    require '../bower_components/skeleton/css/normalize.css'
    require '../bower_components/skeleton/css/skeleton.css'

Next, it's time to get our application up and running, which mostly means calling `require` a lot.
...
view. This is just an example. All the components needed for this root view have been scaffolded out with stub HTML,
to provide a template for the recommended architecture.

This root view, like any complex Web application, uses a layout. Let's `require` that first. Note that `withLayout` is
