###
  ...
  You should use this *very* sparingly, however. If you do have any particular data you want to store, think very
  carefully, because it should probably go in `app/state` instead. the `data` parameter is only for view middlewares
  to pass context to their child views, or similar such uses.

  In general, only use `data` if you are trying to make a reusable partial view or other component.
  ...
  defroute(re, params...)
    function which creates and returns a routing middleware. (Yes, this is a third-order function)
  ...
  defmulti(routes...)
    sugar function that accepts any number of route vectors, converts them into views with `defroute`,
  ...
  After working through over a hundred lines of comments, you might be surprised to see that this component only has
